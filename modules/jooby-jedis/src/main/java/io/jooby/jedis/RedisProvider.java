...
class RedisProvider implements SneakyThrows.Runnable, AutoCloseable {

  /** The logging system. */
  private final Logger log = LoggerFactory.getLogger(RedisProvider.class);
  ...
  public RedisProvider(final Pool<Jedis> pool, final URI uri, final GenericObjectPoolConfig config) {
    this.pool = pool;
    this.uri = uri;
    this.config = config;
  }
